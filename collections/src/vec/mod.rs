...
use super::utils::{alloc_storage_read, append_slice, CacheEntry, EntryState, OnceCell, StableMap};
use nesdie::env;
...
fn expect_consistent_state<T>(val: Option<T>) -> T {
    val.unwrap_or_else(|| unreachable!())
}
...
/// # Examples
/// ```ignore
/// use nesdie::store::Vector;
///
/// let mut vec = Vector::new(Box::new([0xfa]));
/// assert!(vec.is_empty());
///
/// vec.push(1u8);
/// vec.push(2);
    ...
    T: BorshSerialize + BorshDeserialize,
{
    ...
    pub fn get(&self, index: u32) -> Option<&T> {
        let entry = self.cache.get(index).get_or_init(|| {
            let value = alloc_storage_read(&self.index_to_lookup_key(index))
    ...
    fn get_mut_inner(&mut self, index: u32) -> Option<&mut CacheEntry<T>> {
        ...
        let entry = self.cache.get_mut(index);
        entry.get_or_init(|| {
            let value = alloc_storage_read(&index_to_lookup_key)
                .map(|bytes| Self::deserialize_element(&bytes));
            CacheEntry::new_cached(value)
        });
