use crate::utils::types::{Balance, Gas, PromiseIndex};
...
impl PromiseAction {
    pub fn add(&self, promise_index: PromiseIndex) {
        use PromiseAction::*;
        match self {
            CreateAccount => crate::helper_env::promise_batch_action_create_account(promise_index),
            DeployContract { code } => {
                crate::helper_env::promise_batch_action_deploy_contract(promise_index, &code)
            }
            FunctionCall {
                method_name,
                arguments,
                amount,
                gas,
            } => crate::helper_env::promise_batch_action_function_call(
                promise_index,
                &method_name,
                &arguments,
                *amount,
            ...
            Transfer { amount } => {
                crate::helper_env::promise_batch_action_transfer(promise_index, *amount)
            }
            Stake { amount, public_key } => {
                crate::helper_env::promise_batch_action_stake(promise_index, *amount, public_key)
            }
            AddFullAccessKey { public_key, nonce } => {
                crate::helper_env::promise_batch_action_add_key_with_full_access(
                    promise_index,
                    public_key,
                    *nonce,
                )
            ...
            AddAccessKey {
                ...
                method_names,
                nonce,
            } => crate::helper_env::promise_batch_action_add_key_with_function_call(
                promise_index,
                public_key,
                *nonce,
                *allowance,
            ...
            DeleteKey { public_key } => {
                crate::helper_env::promise_batch_action_delete_key(promise_index, public_key)
            }
            DeleteAccount { beneficiary_id } => {
                crate::helper_env::promise_batch_action_delete_account(
                    promise_index,
                    beneficiary_id,
                )
            }
...
impl PromiseSingle {
    pub fn construct_recursively(&self) -> PromiseIndex {
        ...
        let promise_index = if let Some(after) = self.after.borrow().as_ref() {
            crate::helper_env::promise_batch_then(after.construct_recursively(), &self.account_id)
        } else {
            crate::helper_env::promise_batch_create(&self.account_id)
        };
...
impl PromiseJoint {
    pub fn construct_recursively(&self) -> PromiseIndex {
        ...
        let res = crate::helper_env::promise_and(&[
...
impl Promise {
    ...
    fn construct_recursively(&self) -> PromiseIndex {
        ...
        if *self.should_return.borrow() {
            crate::helper_env::promise_return(res);
        }
        res
    }
}
